...
| `finhay-risk-exposure` | Guardrailed workflow: full risk-exposure scan — all 5 risk groups checked (concentration, cash/liquidity, margin/debt, symbol/event, market context) incl. pending orders; every finding carries evidence + severity; clean groups explicitly marked as checked. Reports only — no advice (hands off to `finhay-portfolio-review`). **Read-only**, answers in Vietnamese. | When the user asks what risks their portfolio is carrying or for a thorough risk check/scan. |
| `finhay-cash-allocation` | Guardrailed workflow: cash-allocation analysis — real cash picture (balance, T+ receivables, cash committed by pending orders, margin debt → true available cash), position vs three reference frames (conservative/balanced/aggressive), mechanical what-if scenarios, and the questions the user must answer. Never picks a frame, never suggests products. **Read-only**, answers in Vietnamese. | When the user asks whether their cash is too much/too little, how much cash to hold, or about idle cash. |
