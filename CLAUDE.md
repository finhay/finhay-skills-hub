...
| finhay-cash-allocation | **Workflow** (read-only): cash-allocation analysis — real cash picture (T+ receivables, pending-order commitments, margin debt → true available), position vs 3 reference frames, mechanical scenarios, questions the user must answer; never picks a frame or suggests products | "Is my cash too much/too little", "how much cash should I hold" |
