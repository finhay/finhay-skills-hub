...
- All write endpoints also require the ordering-device identifier header `device-id` — `./finhay.sh request` attaches it automatically on every request (machine fingerprint → SHA-256 → 20 hex, cached at `~/.finhay/credentials/.device-id`, override via `FINHAY_DEVICE_ID`). Never ask the user for it.
